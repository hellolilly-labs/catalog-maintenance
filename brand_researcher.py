...
from src.workflow.workflow_state_manager import get_workflow_manager, WorkflowState, WorkflowState
...
class EnhancedBrandResearcher:
    ...
    def __init__(self):
        ...
        self.researchers = {
            "foundation_research": get_foundation_researcher(),
            "market_positioning": get_market_positioning_researcher(),
            "product_style": get_product_style_researcher(),
            "customer_cultural": get_customer_cultural_researcher(),
            "voice_messaging": get_voice_messaging_researcher(),
            "interview_synthesis": get_interview_synthesis_researcher(),
            "linearity_analysis": get_linearity_analysis_researcher(),
            "research_integration": get_research_integration_processor()
        }
        
        # Map phase names to researcher methods
        self.research_methods = {
            "foundation_research": "research_foundation",
            "market_positioning": "research_market_positioning",
            "product_style": "research_product_style",
            "customer_cultural": "research_customer_cultural",
            "voice_messaging": "research_voice_messaging",
            "interview_synthesis": "research_interview_synthesis",
            "linearity_analysis": "research_linearity_analysis",
            "research_integration": "integrate_research"
        }
    ...
    async def run_research_phase(self, brand_domain: str, phase_key: str, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        self.workflow_manager.start_research_phase(brand_domain, phase_key)
        ...
        try:
            ...
            result = await research_method(brand_domain, force_refresh=force_refresh)
            duration = time.time() - start_time
            
            # Update workflow state to complete
            quality_score = result.get("quality_score", 0.75)
            self.workflow_manager.complete_research_phase(
            ...
            # Update workflow state to failed - fix WorkflowState reference
    ...
    async def run_all_research_phases(self, brand_domain: str, force_refresh: bool = False, 
        ...
        for phase_key in phase_order:
            try:
                ...
                if not force_refresh and continue_from_current:
                    phase_status = self.workflow_manager.get_research_phase_status(brand_domain, phase_key)
        ...
        progress_summary = self.workflow_manager.get_research_progress_summary(brand_domain)
    ...
    async def continue_research_pipeline(self, brand_domain: str, force_refresh: bool = False) -> Dict[str, Any]:
        ...
        total_duration = time.time() - start_time
        final_progress = self.workflow_manager.get_research_progress_summary(brand_domain)
    ...
    def get_brand_status(self, brand_domain: str) -> Dict[str, Any]:
        """Get comprehensive brand research status"""
        progress_summary = self.workflow_manager.get_research_progress_summary(brand_domain)
        current_state = self.workflow_manager.get_brand_state(brand_domain)
        
        # Get phase-by-phase status
        phase_statuses = {}
        for phase_key in self.workflow_manager.get_required_research_phases().keys():
            phase_statuses[phase_key] = self.workflow_manager.get_research_phase_status(brand_domain, phase_key)
        ...
        return {
            ...
            "next_step": self.workflow_manager.get_next_step(brand_domain)
        }
...
async def main():
    ...
    researcher = EnhancedBrandResearcher()
    ...
    try:
        ...
        if args.status:
            # Show brand status
            status = researcher.get_brand_status(args.brand)
